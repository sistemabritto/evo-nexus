...
def build(reward_url):
    ...
    reward += '<section><h2>Quer transformar isso em escopo?</h2><p>A Consultoria a Laser é uma oferta paga para organizar prioridades e o PRD do projeto. Não inclui a implementação. Consulte condições e horários na página; avançar é opcional.</p><a class="button" href="https://www.sistemabritto.com.br/sessao-de-arquitetura?utm_source=instagram&amp;utm_medium=material&amp;utm_campaign=gambito_v4">Conhecer a Consultoria a Laser</a></section>'
    token=reward_url.rstrip('/').split('/')[-1].split('?')[0]
    public_base=reward_url.split('/share/')[0]
    destination='https://www.sistemabritto.com.br/sessao-de-arquitetura?'+urlencode(dict(utm_source='instagram',utm_medium='material',utm_campaign='gambito_v4'))
    tracked=public_base+'/api/shares/'+token+'/click?'+urlencode({'to':destination,'label':'gambito_v4_caderno_laser'})
    reward=reward.replace('href="https://www.sistemabritto.com.br/sessao-de-arquitetura?utm_source=instagram&amp;utm_medium=material&amp;utm_campaign=gambito_v4"',f'target="_blank" rel="noopener" href="{e(tracked)}"')
    ...
    for r, alt in zip(REELS, ALTS):
        ...
        if key=='LASER':
            url='https://www.sistemabritto.com.br/sessao-de-arquitetura?'+utm
            gift='as condições e os horários da Consultoria a Laser, uma oferta paga de diagnóstico e PRD'
            cta='Quer organizar prioridades e sair com o PRD do seu projeto? Comenta LASER para conhecer a Consultoria a Laser. É uma consultoria paga; a implementação é uma próxima etapa.'
        else:
